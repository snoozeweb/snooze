  ...
  <div>
    ...
    >
      ...
      <div v-else-if="ctype == 'weekdays'" class="d-flex align-items-center">
        <CBadge style="font-size: 0.875rem;" color="warning" v-for="(weekday, ind) in date_obj.weekdays" :key="ind" :class="ind != date_obj.weekdays.length - 1 ? 'me-1' : ''">{{ get_weekday(weekday) }}</CBadge>
      </div>
