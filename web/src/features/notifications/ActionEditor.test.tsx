...
describe("ActionEditor", () => {
  it("creates a new action with valid JSON on Save", async () => {
    ...
    await user.type(screen.getByLabelText(/^name$/i), "slack-prod");
    await user.click(screen.getByRole("button", { name: /create/i }));
    await waitFor(() => expect(bodies).toHaveLength(1));
    expect((bodies[0] as { name: string }).name).toBe("slack-prod");
