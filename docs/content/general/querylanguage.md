...
It's possible to use strings by quoting them when they contain special characters. Single quotes and double quotes supported. Special characters can be escaped with `\\` when using quotes.

``` console
host = "myhost01"
```

\> Note: Only expressions composed of alphanumerics, `_`, `-` and `.` are \> supported when omitting quotes.
