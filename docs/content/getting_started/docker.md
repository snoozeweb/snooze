...
Snooze 2.0 publishes one distroless image per Go binary at `snoozeweb/snooze:latest` (server) or `snoozeweb/snooze-<binary>:latest` (components). The most common starting points are documented below; the repository `docker-compose.yaml` covers each layout end-to-end with named profiles.
...
The Web interface is then available at `http://localhost:5200`. This is the lowest-friction layout: no external database, no replica set, zero infrastructure.
...
The repository `docker-compose.yaml` is profiled so you can pick a backend at `up` time:
...
The `mongo` profile is the canonical multi-replica layout: three `snooze-server` containers behind an nginx `round_robin` load balancer talking to a 3-node replica set, with the bus / syncer piggy-backed on Mongo change streams.

## Helm

A Helm chart lives at `packaging/helm/`. Set `database.kind` to pick the backend:
