...
##################
    ...
    :Package location: ``/etc/snooze/server-go/ldap.yaml`` (Go canonical)
    :Legacy name: ``/etc/snooze/server/ldap_auth.yaml`` (still loaded)
    :Loader: ``internal/config`` (koanf)
    :Live reload: ``False`` (restart the server to re-read)

Configuration for the LDAP authentication backend. Only consulted
when ``enabled: true``; the LDAP provider is otherwise not
registered in ``buildAuthProviders``.
