...
#########################
    ...
    :Package location: ``/etc/snooze/server-go/housekeeper.yaml`` (Go canonical)
    :Legacy name: ``/etc/snooze/server/housekeeping.yaml`` (still loaded)
    :Loader: ``internal/config`` (koanf)
    :Live reload: ``False``

Configuration for the housekeeper goroutine: TTLs and the cadence at
which orphan / expired records are reaped. Durations accept both bare
seconds (legacy Python form) and Go-style strings such as ``"5m"`` or
``"24h"``.
