...
#####################
    ...
    :Package location: ``/etc/snooze/server-go/general.yaml``
    :Loader: ``internal/config`` (koanf)
    :Live reload: ``False`` (the YAML is bootstrap-only)
    :Runtime store: ``settings`` plugin (``PATCH /api/v1/settings/{key}``)

General configuration of snooze. The YAML file seeds the defaults
read by the server at startup; thereafter the runtime-mutable values
live in the ``settings`` collection in the database and are edited
through the WebUI or the REST API.
