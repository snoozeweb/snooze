...
func (h *testHost) Tracer() trace.Tracer         { return otel.Tracer("widget-test") }
...
func TestRegistration(t *testing.T) {
	require.True(t, slices.Contains(plugins.Registered(), "widget"))
}

func TestPostInitRoundtrip(t *testing.T) {
	host := newTestHost(t)
	p := &Plugin{meta: plugins.Metadata{Name: "widget"}}
...
func TestValidate(t *testing.T) {
	...
	require.NoError(t, p.Validate(map[string]any{"name": "load"}))
}
